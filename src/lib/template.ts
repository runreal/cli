...
export function render(input: string[], cfg: RunrealConfig): string[] {
	...
	return input.map((arg) => replace(placeholderRegex, arg, substitutions))
}

const replace = (regex: RegExp, item: string, substitutions: Record<string, string | undefined>) => {
...
export function renderConfig(cfg: RunrealConfig) {
	const substitutions: Record<string, string | undefined> = getSubstitutions(cfg)
	return renderItems(cfg, substitutions, replace) as RunrealConfig
}
